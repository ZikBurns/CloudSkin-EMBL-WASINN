...
fn main_entry() {
    ...
    infer_image(model_name,&image_name);
...
*/
    ...
    let numpy_array: Array3<f32> = read_npy("data_array.npy").expect("Error reading npy file");
    let first_element = numpy_array.index_axis(ndarray::Axis(0), 0);
    println!("First 5 elements: {:?}", first_element);
    ...
    println!("Flattened Array Length: {}", flattened.len());
