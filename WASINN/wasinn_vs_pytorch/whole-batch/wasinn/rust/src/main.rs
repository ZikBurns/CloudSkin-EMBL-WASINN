...
use ndarray::{Array, Array1, Array2,Array3,Array4, Ix3,ArrayView3, Axis};
...
fn main_entry() {
    ...
    let images_npy: &str = &args[2]; // File name for the input image

    infer_image(&model_name,&images_npy);

}

fn infer_image(model_bin_image:&str, images_npy: &str) {
...
*/
    ...
    let numpy_array: Array4<f32> = read_npy(images_npy).expect("Error reading npy file");
    // Get the total number of elements
    let num_elements = numpy_array.len();
    println!("Num elements: {:?}", num_elements);
    let first_dim = numpy_array.shape()[0];
    println!("1 dim: {:?}", first_dim);
    let first_element = numpy_array.index_axis(ndarray::Axis(0), 0);
    println!("First element: {:?}", first_element);
    for d in 0..first_dim{
        let array3: Array3<f32> = numpy_array.index_axis(Axis(0), d).to_owned();
        let num_elements = array3.len();
        //println!("Num elements: {:?}", num_elements);
        let first_dim = array3.shape()[0];
        //println!("1 dim: {:?}", first_dim);
        let first_element = array3.index_axis(ndarray::Axis(0), 0);
        //println!("First element: {:?}", first_element);
        let second_dim_size = num_elements / 3;
        let reshaped_array: Array2<f32> = array3.into_shape((3, second_dim_size)).expect("Error reshaping array");
    ...
    println!("Images number: {:?}", num_images);


    //
    //
    // if let Ok(entries) = fs::read_dir(images_npy) {
